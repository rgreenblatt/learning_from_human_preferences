...
def optional_task_by_name(name, short=False):
    if name == "reacher":
...
def make_with_torque_removed(env_id):
    ...
    if env_id.startswith('short'):
        ...
        short = False
    return optional_task_by_name(env_id, short)  # Use our task_by_name function to get the env
...
def humanoid(standup=True, short=False):
    ...
    if standup:
        bonus = lambda _, data: 5 * (data.qpos[2, 0] - 1)
        env = NeverDone(env, bonus=bonus)
    return limit(env, 300 if short else 1000)

def double_pendulum():
    bonus = lambda _, data: 10 * (data.site_xpos[0][2] - 1)
...
def pendulum():
    # bonus = lambda _, data: np.concatenate([data.qpos, data.qvel]).ravel()[1] - 1.2
    def bonus(_, data):
        angle = data.qpos[1, 0]
